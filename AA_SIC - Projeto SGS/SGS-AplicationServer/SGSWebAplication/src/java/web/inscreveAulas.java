...
public class inscreveAulas extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(session.getAttribute("Type")!= null && session.getAttribute("Type").equals("Administrador")){
            sgs.Aula aula = aulaBean.getAula((String) request.getParameter("codAula"));
            request.setAttribute("Aula", aula);
            List<sgs.Aluno> allAlunos = disciplinaBean.getAllAlunosPerDiscp(aula.getDisciplina().getCodigo());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(session.getAttribute("Type")!= null && session.getAttribute("Type").equals("Administrador")){
            ...
            if(json.get(0).equals("Inscreve")){
                message = aulaBean.inscreveAula(json.get(1), json.get(2));
                if(message.equals("true")) writer.println(message);
            }
            else if(json.get(0).equals("Desinscreve")){
                message = aulaBean.desinscreveAula(json.get(1), json.get(2));
