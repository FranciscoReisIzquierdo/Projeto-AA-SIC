...
public class docenteManager extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(session.getAttribute("Type")!= null && session.getAttribute("Type").equals("Administrador")){
            ...
            request.setAttribute("allDocentes", allDocentes);
            request.getRequestDispatcher("/WEB-INF/docenteManager.jsp").forward(request, response);
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(session.getAttribute("Type")!= null && session.getAttribute("Type").equals("Administrador")){
            ...
            if(json.get(0).equals("Edit")){
                docenteBean.updateDocente(json.get(1), json.get(2), Integer.parseInt(json.get(3)), 
                        json.get(4), "", json.get(6), json.get(5).charAt(0));
            }
            else if(json.get(0).equals("Delete")) docenteBean.deleteDocente(json.get(1));
        }
    }
