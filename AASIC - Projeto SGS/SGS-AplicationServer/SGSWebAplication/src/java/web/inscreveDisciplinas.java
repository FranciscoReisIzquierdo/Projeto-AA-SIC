...
public class inscreveDisciplinas extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(session.getAttribute("Type")!= null && session.getAttribute("Type").equals("Aluno")){
            ...
            sgs.Aluno aluno = (sgs.Aluno) userFunctions.getUtilizador((String) session.getAttribute("Email"));
            session.setAttribute("Aluno", aluno);
            sgs.DisciplinaFunctions discpFunctions = new sgs.DisciplinaFunctions();
            List<sgs.Disciplina> allDiscps = discpFunctions.getAllDisciplinasPerCurso(aluno.getCurso().getCodigo());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(session.getAttribute("Type")!= null && session.getAttribute("Type").equals("Aluno")){
            ...
            if(json.get(0).equals("Inscreve")){
                message = discpFunctions.inscreveDisciplina(json.get(1), json.get(2));
                if(message.equals("true")) writer.println(message);
            }
            else if(json.get(0).equals("Desinscreve")){
                message = discpFunctions.desinscreveDisciplina(json.get(1), json.get(2));
